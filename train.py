...
def grid_image(np_images, gts, preds, n=16, shuffle=False):
    ...
    for idx, choice in enumerate(choices):
        ...
        title = "\n".join([f"{task} - gt: {gt_label}, pred: {pred_label}" for gt_label, pred_label, task in zip(gt_decoded_labels, pred_decoded_labels, tasks)])
...
class Lite(LightningLite):
    ...
    def run(self, data_dir, model_dir, args):
        ...
        transform_module = getattr(import_module("dataset"), args.augmentation)  # default: MaskSplitByProfileDataset
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_dir', type=str, default=f'./result/{args.model}')
    ...
    Lite(
        ...
        precision=16,
        gpus=1,
    ).run(data_dir, model_dir, args)

    print(f"Training Finished : {args.name}")